...
struct segTree {
  ...
  void setValue(int ind, int val, int l = 1, int r = 1000001, int v = 1) {
    if (l == r)
      value[v] = value[v] > val ? value[v] : val;
    else {
      int h = (l + r) / 2;
      if (ind <= h)
        setValue(ind, val, l, h, 2 * v);
      else
        setValue(ind, val, h + 1, r, 2 * v + 1);
      value[v] =
          value[2 * v] > value[2 * v + 1] ? value[2 * v] : value[2 * v + 1];
    }
  }
...
int main() {
  ...
  for (int i = 0; i < query[0].size(); i++)
    if (query[0][i].first == 1) S.insert(query[0][i].second);
  int ans;
  if (!S.empty()) {
    it = S.end();
    it--;
  ...
  for (int i = 1; i <= 1000001; i++) {
    for (int j = 0; j < query[i].size(); j++) {
      if (query[i][j].first == 1)
        S.insert(query[i][j].second);
      else {
