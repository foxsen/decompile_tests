...
int chk[200005], dpth[200005], col[200005], pos[200005], light[200005];
...
int query(int node, int b, int e, int i, int j, int carry) {
  ...
  int l = 2 * node, r = l + 1, m = (b + e) / 2;
  return query(l, b, m, i, j, carry + tree[node].carry) +
         query(r, m + 1, e, i, j, carry + tree[node].carry);
}
int main() {
  int i, j, k;
  int n;
  ...
  sort(ara + 1, ara + n + 1);
  for (i = 1; i <= n; ++i)
    pos[ara[i].second] = i, light[i] = col[ara[i].second];
  int q;
  scanf("%d", &q);
  init(1, 1, n);
  while (q--) {
    scanf("%s %d", str, &k);
    if (str[0] == 'p')
      update(1, 1, n, pos[k], pos[k] + dpth[k] - 1, 0);
    else
