...
void get_time(point w) {
  if (ORI(L1, L2) != 0) {
    ...
    if (T3 < 0 && abs(T3) < 1E-7) T3 = 0;
    if (T3 < 0) return;
    T1 = point(w - WIND * T3 - BEG) / point(X1, Y1);
    if (T1 < 0 && abs(T1) < 1E-7) T1 = 0;
    if (T1 < 0) return;
    T2 = (T1 * Z1 + T3 * Z2) / (-D);
    if (T2 < 0 && abs(T2) < 1E-7) T2 = 0;
    if (T2 < 0) return;
    if (t1 > T1 || (abs(t1 - T1) < 1E-7 && t2 > T2)) {
      t1 = T1;
      t2 = T2;
    }
  } else {
