...
void dfs2(int u, int f) {
  ...
  for (register int i = head[u]; i; i = G[i].next) {
    ...
    if (g[trueid[v]].size() < g[trueid[u]].size()) {
      for (it = g[trueid[v]].begin(); it != g[trueid[v]].end(); ++it)
        g[trueid[u]].insert(*it);
      g[trueid[v]].clear();
    } else {
      for (it = g[trueid[u]].begin(); it != g[trueid[u]].end(); ++it)
        g[trueid[v]].insert(*it);
      g[trueid[u]].clear();
      trueid[u] = trueid[v];
    }
  }
  if (u == 1) return;
  for (register unsigned i = 0; i < del[u].size(); i++)
    g[trueid[u]].erase(g[trueid[u]].find(del[u][i]));
  if (g[trueid[u]].size() == 0)
    ans[id[u]] = 1e9;
  else
    ans[id[u]] = *g[trueid[u]].begin();
}
int main() {
  read(n);
  read(m);
  ...
  for (register int i = 1; i <= m; i++) {
    if (a[i].tag == 0) {
      ...
      g[a[i].u].insert(a[i].val);
      g[a[i].v].insert(a[i].val);
      del[d].push_back(a[i].val);
      del[d].push_back(a[i].val);
    }
  }
  dfs2(1, 0);
