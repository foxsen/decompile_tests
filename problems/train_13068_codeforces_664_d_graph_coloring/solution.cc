...
bool dfs(const Graph& g, int color, Edge e, int flg, vi& vis, vi& use) {
  if (vis[e.dst] != -1) return (color == e.color) == (vis[e.src] == vis[e.dst]);
  vis[e.dst] = flg;
  if (flg) use.push_back(e.dst);
  for (Edge e2 : g[e.dst])
    if (!dfs(g, color, e2, flg == (e2.color == color), vis, use)) return false;
  return true;
}
bool solve(Graph g, int color, vi& res) {
  ...
  for (int i = int(0); i < int(n); i++)
    if (vis1[i] == -1) {
      ...
      if (ok1 && ok2)
        use = use1.size() < use2.size() ? use1 : use2;
      else if (ok1)
        use = use1;
      else if (ok2)
        use = use2;
        ...
        return false;
      res.insert(end(res), begin(use), end(use));
    }
  return true;
}
int main() {
