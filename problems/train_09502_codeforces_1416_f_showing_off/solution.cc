...
struct MaxFlow {
  ...
  T dfs_dinic(int v, int s, T f) {
    if (v == s) return f;
    T res = 0;
    int level_v = level[v];
    for (int &i = iter[v]; i < (int)edges[v].size(); i++) {
      edge &e = edges[v][i];
      if (level_v <= level[e.to] or edges[e.to][e.rev].cap == 0) continue;
      T d = dfs_dinic(e.to, s, std::min(f - res, edges[e.to][e.rev].cap));
      if (d <= 0) continue;
      e.cap += d;
  ...
  void add_edge(int from, int to, T capacity) {
    int fid = edges[from].size(), tid = edges[to].size() + (from == to);
    edges[from].push_back(edge{to, tid, capacity});
    edges[to].push_back(edge{from, fid, 0});
  }
  T Dinic(int s, int t, T req) {
    ...
    while (req > 0) {
      ...
      T f;
      while ((f = dfs_dinic(t, s, req)) > 0) flow += f, req -= f;
    }
    return flow;
  }
...
struct MaxFlowLowerBound {
  ...
  T flush(int s, int t) {
    ...
    mf.add_edge(t, s, INF);
    if (mf.Dinic(N, N + 1) < sum) return -1;
    return INF - mf.edges[t].back().cap + mf.Dinic(s, t);
  }
};
...
char dir(int x, int y, int nx, int ny) {
  string ret = "DURL";
  for (int d = (0), d_end_ = (4); d < d_end_; d++) {
...
void solve() {
  ...
  for (int t = (0), t_end_ = (v2ij.size()); t < t_end_; t++)
    if (v2ij[t].size()) {
      ...
      for (int i = (0), i_end_ = (vs.size()); i < i_end_; i++) {
        ...
        bool parity = (x + y) % 2;
        if (parity)
          mf.add_edge(vs.size(), i, 1, 1);
        else
          mf.add_edge(i, vs.size() + 1, 1, 1);
        for (int d = (0), d_end_ = (4); d < d_end_; d++) {
          int nx = x + dx[d];
          int ny = y + dy[d];
          if (nx >= 0 and ny >= 0 and nx < H and ny < W) {
            ...
            if (A[x][y] > A[nx][ny] and !flg) {
              flg = true;
              if (parity)
                mf.add_edge(i, vs.size() + 1, 0, 1);
              else
                mf.add_edge(vs.size(), i, 0, 1);
            }
          }
        }
      }
      ...
      for (int i = (0), i_end_ = (vs.size()); i < i_end_; i++) {
        ...
        for (auto &e : mf.mf.edges[i]) {
          ...
          } else if ((parity and e.to == vs.size() + 1 and e.cap == 0) or
                     (!parity and e.to == vs.size() and e.cap == 1)) {
            for (int d = (0), d_end_ = (4); d < d_end_; d++) {
              int nx = x + dx[d];
              int ny = y + dy[d];
              if (nx >= 0 and ny >= 0 and nx < H and ny < W) {
  ...
  cout << "YES\n";
  for (int i = (0), i_end_ = (H); i < i_end_; i++) {
    for (int j = (0), j_end_ = (W); j < j_end_; j++) cout << ret[i][j] << ' ';
    cout << '\n';
  }
  for (int i = (0), i_end_ = (H); i < i_end_; i++) {
    for (int j = (0), j_end_ = (W); j < j_end_; j++) cout << cret[i][j] << ' ';
    cout << '\n';
  }
}
int main() {
