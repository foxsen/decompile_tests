...
bool mark_path(int node, int anc, status target) {
  while (node >= 0 && depth[node] > depth[anc]) {
    if (edge_status[node] != target && edge_status[node] != NONE) return false;
...
int main() {
  ...
  for (int i = 0; i < N; i++)
    for (edge &e : adj[i])
      if (!is_bridge[e.index]) biconnected_UF.unite(i, e.node);
  lca.init(N);
  ...
  for (int i = 0; i < N; i++)
    for (edge &e : adj[i])
      if (is_bridge[e.index]) {
        ...
        if (tree_UF.unite(a, b)) lca.add_edge(a, b);
      }
  lca.build();
  ...
  for (int q = 0; q < Q; q++) {
    ...
    if (anc < 0 || !mark_path(source, anc, UP) ||
        !mark_path(destination, anc, DOWN)) {
      cout << "No" << '\n';
      return 0;
    }
  }
