...
void solve_cnt() {
  ...
  for (int i = 0; i < 100; i++)
    for (int j = 0; j < M; j++)
      for (int k = 0; k < D; k++)
        cnt[i][k][((j + gen[k]) % M) / 3][1 << ((j + gen[k]) % M % 3)] =
            cnt_sum[i][j];
  for (int i = 0; i < 100; i++)
    for (int k = 0; k < D; k++)
      for (int j = 0; j < 20; j++) {
  ...
  for (int i = 0; i <= 100; i++)
    for (int j = 0; j < 20 * 8; j++)
      for (int k = 1; k < D; k++)
        cnt[i][k][j / 8][j % 8] += cnt[i][k - 1][j / 8][j % 8];
}
vector<cat> masks_pos[101][20];
vector<cat> masks_neg[101][20];
cat ans = 0;
cat __attribute__((optimize("Ofast,unroll-loops")))
fast_inner_loop(const cat* __restrict masks_ptr,
                const cat* __restrict cnt_ptr) {
  cat ret = 0;
...
void solve_leaf(cat A_mask, cat Xl, cat Xr) {
  ...
  while (!Vl.empty() && !Vr.empty() && Vl.size() == Vr.size() &&
         Vl.back() == Vr.back()) {
    int s = gen[Vl.back()];
...
void solve_recursive(vector<cat>& A_mask, cat Xl, cat Xr) {
  ...
  for (int i = 0; i < D; i++) {
    ...
    for (int j = 0, k = i, l = 0; j < N; j++, k++) {
      ...
      int g = gen[k];
      cat mask_new =
          (A_mask[j] >> g) | ((A_mask[j] & ((1LL << g) - 1)) << (M - g));
      if ((int)A_mask_new.size() == l) A_mask_new.push_back((1LL << M) - 1);
      A_mask_new[l] &= mask_new;
    }
    if (A_mask_new.size() == 1)
      solve_leaf(A_mask_new[0], (Xl - 1 - i >= 0) ? (Xl - 1 - i) / D + 1 : 0,
                 (Xr - 1 - i) / D + 1);
    else
      solve_recursive(A_mask_new, (Xl - 1 - i >= 0) ? (Xl - 1 - i) / D + 1 : 0,
                      (Xr - 1 - i) / D + 1);
  }
}
void solve(vector<int>& B, cat Xl, cat Xr) {
  ...
  if (N == 1)
    solve_leaf(A_mask[0], Xl, Xr);
  else
    solve_recursive(A_mask, Xl, Xr);
}
int main() {
  ...
  for (int i = 0; i <= 100; i++)
    for (int j = 0; j < D; j++) {
      masks_pos[i][j].resize(1000, 0);
      ans += fast_inner_loop(masks_pos[i][j].data(), &(cnt[i][j][0][0]));
    }
  for (int i = 0; i <= 100; i++)
    for (int j = 0; j < D; j++) {
      masks_neg[i][j].resize(1000, 0);
