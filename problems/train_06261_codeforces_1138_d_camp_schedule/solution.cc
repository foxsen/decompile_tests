#include <bits/stdc++.h>
using namespace std;
long long hash1 = 1, hash2 = 1, mod1 = 1000000007, mod2 = 1000000009,
          stala1 = 13, stala2 = 17;
int conv(char a) { return int(a) - 48; }
...
int main() {
  ...
  for (int x = b.size() - 1; x >= 0; x--) {
    if (pref1[x] == suf1[b.size() - x + 1] &&
        pref2[x] == suf2[b.size() - x + 1]) {
      maks = x;
      break;
    }
  }
  for (int x = 0; x < a.size(); x++) licz[conv(a[x])]++;
  maks = b.size() - maks;
