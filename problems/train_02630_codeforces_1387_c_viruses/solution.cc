...
class AhoCorasick {
 public:
  ...
  bool IsBadPrefix(int x) { return bad[x]; }
};
int main() {
  ...
  vector<vector<vector<unsigned long long>>> dist(
      adj.size(), vector<vector<unsigned long long>>(
                      ac.sz, vector<unsigned long long>(ac.sz, INF)));
  struct State {
    int u, st, et;
    unsigned long long d;
    State() {}
    State(int u, int st, int et, unsigned long long d)
        : u(u), st(st), et(et), d(d) {}
