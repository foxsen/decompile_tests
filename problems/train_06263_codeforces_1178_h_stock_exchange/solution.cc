...
int a[4405], b[4405];
long long g[N], f[N], jb[N], jj[N];
bool spfa(int s, int t, int n) {
  for (int i = 1; i <= n; i++) {
    dist[i] = inf;
    last[i] = laedge[i] = vis[i] = que[i - 1] = 0;
  }
  int tot = 1, l = 0, r = 0;
  que[0] = s;
  vis[s] = true;
...
bool cmg(int x, int y) { return g[x] < g[y]; }
...
void solve(long long x) {
  ...
  for (int i = (int)(n + 1); i <= (int)(2 * n); i++) g[q[i] = i] = b[i];
  sort(q + n + 1, q + 2 * n + 1, cmg);
  for (int i = (int)(n + 1); i <= (int)(2 * n); i++) {
    ins(q[i] + n, q[i], 1, 0);
    if (i > n + 1) ins(q[i] + n, q[i - 1] + n, inf, 0);
    ins(i, t, 1, 0);
  }
  for (int i = (int)(1); i <= (int)(n); i++) {
    for (int j = (int)(2 * n); j >= (int)(n + 1); j--)
      if (g[q[j]] <= b[i]) {
        ins(i, q[j] + n, 1, 0);
        break;
      }
