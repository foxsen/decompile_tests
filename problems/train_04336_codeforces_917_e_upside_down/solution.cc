#include <bits/stdc++.h>
using namespace std;
int n, m, Q, E, cnt, cntpoi, tree[200001], ret[200001];
int to[200001], nex[200001], fir[200001], w[200001], pa[200001];
int st[200001], ed[200001], size[200001], co[200001], dep[200001];
int acori[200001], acrev[200001];
vector<int> vec[200001];
bool vis[200001];
int posori[200001], posrev[200001];
struct point {
  int x, y, t;
...
bool operator<(query a, query b) { return a.wei < b.wei; }
bool comz(query a, query b) { return a.z < b.z; }
struct mzc {
  int NODE, SAMNODE, TIME, acTIME;
  int c[200001][26], lis[200001], tr[200001][26], fir[200001], nex[200001];
  int mx[200001], poi[200001], lp[200001], rp[200001], str[200001],
      from[200001];
  int fail[200001], fa[200001], ch[200001][26], aclp[200001], acrp[200001];
  ...
  int extend(int p, int c, int pos) {
    ...
    if (!p)
      fa[np] = 1;
    else if (mx[q = tr[p][c]] == mx[p] + 1)
      fa[np] = q;
    else {
      mx[nq = ++SAMNODE] = mx[p] + 1;
      from[nq] = pos;
  ...
  void dfs(int now) {
    lp[now] = ++TIME;
    for (int i = 0; i < 26; i++)
      if (ch[now][i]) dfs(ch[now][i]);
    rp[now] = TIME;
  }
  void acdfs(int now) {
    aclp[now] = ++acTIME;
    for (int i = fir[now]; i; i = nex[i]) acdfs(i);
    acrp[now] = acTIME;
  }
  void build(bool rev) {
    ...
    for (int i = 1; i <= NODE; i++) nex[i] = fir[fail[i]], fir[fail[i]] = i;
    dfs(1);
    acdfs(0);
  }
  mzc() {
...
state trans(state x, int y) {
  if (!x.d1)
    if (x.le1 == ori.mx[x.p1])
      if (ori.ch[x.p1][y])
        x.p1 = ori.ch[x.p1][y], x.le1++;
      else
        x.d1 = 1;
    else if (pa[ori.from[x.p1] - x.le1] == y)
      x.le1++;
    else
      x.d1 = 1;
  if (!x.d2)
    if (x.le2 == rev.mx[x.p2])
      if (rev.ch[x.p2][y])
        x.p2 = rev.ch[x.p2][y], x.le2++;
      else
        x.d2 = 1;
    else if (pa[rev.from[x.p2] + x.le2] == y)
      x.le2++;
    else
      x.d2 = 1;
  while (x.p3 && !ori.c[x.p3][y]) x.p3 = ori.fail[x.p3];
  if (ori.c[x.p3][y]) x.p3 = ori.c[x.p3][y];
  while (x.p4 && !rev.c[x.p4][y]) x.p4 = rev.fail[x.p4];
...
int getsize(int now, int fat) {
  ...
  for (int i = fir[now]; i; i = nex[i])
    if (to[i] != fat && !vis[to[i]]) size[now] += getsize(to[i], now);
  return size[now];
}
void paint(int now, int fat, int color, int alpha, int depth) {
  ...
  for (int i = fir[now]; i; i = nex[i])
    if (!vis[to[i]] && to[i] != fat) paint(to[i], now, color, w[i], depth);
}
int oritr[200001], revtr[200001];
...
void acdfs(int now, int fat, int depth) {
  ...
  for (int i = 0; i < vec[now].size(); i++)
    if (vec[now][i] > 0)
      ret[que[vec[now][i]].id] += acquerev(rev.aclp[acrev[que[vec[now][i]].z]],
                                           rev.acrp[acrev[que[vec[now][i]].z]]);
    else
      ret[que[-vec[now][i]].id] +=
          acqueori(ori.aclp[acori[que[-vec[now][i]].z]],
                   ori.acrp[acori[que[-vec[now][i]].z]]);
  vec[now].clear();
  for (int i = fir[now]; i; i = nex[i])
    if (!vis[to[i]] && to[i] != fat) acdfs(to[i], now, depth);
  acaddori(ori.aclp[sta[now][depth].p3], -1);
  acaddrev(rev.aclp[sta[now][depth].p4], -1);
}
void solve(int now, int l, int r, int depth) {
  ...
  int color = 0;
  for (int i = fir[now]; i; i = nex[i])
    if (!vis[to[i]]) paint(to[i], now, ++color, w[i], depth);
  for (int i = l; i <= r; i++)
  ...
  for (int i = fir[now]; i; i = nex[i])
    if (!vis[to[i]]) acdfs(to[i], now, depth);
  if (j > r) return;
  for (int i = fir[now]; i; i = nex[i])
  ...
  for (int i = 1, j = 1; i <= m && j <= Q; i++)
    if (que[j].z == i) {
      ...
      for (int k = st[i]; k < ed[i]; k++)
        Plus(ori.lp[posori[k]], ori.rp[posori[k]], rev.lp[posrev[k + 1]],
             rev.rp[posrev[k + 1]]);
      for (; j <= Q && que[j].z == i; j++) {
        state x = sta[que[j].x][dep[que[j].wei]],
              y = sta[que[j].y][dep[que[j].wei]];
