...
struct LCT {
  ...
  LCT() {
    ...
    memset(sss, 0, sizeof(sss)), memset(ss, 0, sizeof(ss)),
  ...
  void rotate(int x) {
    int p = fa[x], d = ((x) == ch[fa[x]][1]);
    ch[p][d] = ch[x][d ^ 1], fa[ch[x][d ^ 1]] = p;
    if (!(ch[fa[p]][0] != (p) && ch[fa[p]][1] != (p)))
      ch[fa[p]][((p) == ch[fa[p]][1])] = x;
    fa[x] = fa[p];
    ch[x][d ^ 1] = p, fa[p] = x;
    pushup(p), pushup(x);
  }
  void splay(int x) {
    while (!(ch[fa[x]][0] != (x) && ch[fa[x]][1] != (x))) {
      int p = fa[x];
      if (!(ch[fa[p]][0] != (p) && ch[fa[p]][1] != (p)))
        rotate(((x) == ch[fa[x]][1]) == ((p) == ch[fa[p]][1]) ? p : x);
      rotate(x);
    }
    pushup(x);
  }
...
int main() {
  ...
  for (int i = n; i >= 2; i--) {
    ...
    int suc = it == S.end() ? 0 : a[*it],
        pre = it == S.begin() ? 0 : a[*(--it)];
    if (pre)
      lct.access(pre), lct.splay(pre), lct.splay(i), lct.fa[lct.ch[i][1]] = 0,
                                                     lct.splay(pre);
    if (suc)
      lct.access(suc), lct.splay(suc), lct.splay(i), lct.fa[lct.ch[i][1]] = 0,
                                                     lct.splay(suc);
