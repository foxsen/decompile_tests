...
class avlTree {
 ...
 protected:
  ...
  int get_rank(avl T, name x) {
    ...
    while (T != NullNode) {
      if (T->data == x)
        return ans + T->ls->size + 1;
      else if (x < T->data)
        T = T->ls;
      else
        ans += T->ls->size + T->num, T = T->rs;
    }
    return ans;
  }
  name get_data(avl T, int x) {
    while (T != NullNode) {
      if (T->ls->size + T->num < x)
        x -= T->ls->size + T->num, T = T->rs;
      else if (x <= T->ls->size)
        T = T->ls;
      else
        return T->data;
  ...
  auto query = [&](int x, int y) {
    ...
    while (true) {
      ...
      if (y <= pl) {
        ...
        long long ret = -min(invs[o], tot[o] - invs[o]) +
                        qry(ls[o], l, pl, x, y) + qry(rs[o], pl + 1, r, y, x);
        if (p[x] < p[y])
          invs[o] += 1;
        else
          invs[o] -= 1;
