...
ll max_query(int t, int l, int r){
    ...
    if(lid==rid){
        for(int i=l; i<=r; ++i) ret = max(ret, x[t][lid][i%SZ]+ADD[t][lid]);
    }
    else{
        for(int bid = lid+1; bid < rid; ++bid) ret = max(ret, MAX[t][bid]+ADD[t][bid]);

        for(int i=l; i<SZ*(lid+1); ++i) ret = max(ret, x[t][lid][i%SZ]+ADD[t][lid]);
        for(int i=SZ*rid; i<=r; ++i) ret = max(ret, x[t][rid][i%SZ]+ADD[t][rid]);
    }
    return (ll)ret;
}
...
ll min_query(int t, int l, int r){
    ...
    if(lid==rid){
        for(int i=l; i<=r; ++i) ret = min(ret, x[t][lid][i%SZ]+ADD[t][lid]);
    }
    else{
        for(int bid = lid+1; bid < rid; ++bid) ret = min(ret, MIN[t][bid]+ADD[t][bid]);

        for(int i=l; i<SZ*(lid+1); ++i) ret = min(ret, x[t][lid][i%SZ]+ADD[t][lid]);
        for(int i=SZ*rid; i<=r; ++i) ret = min(ret, x[t][rid][i%SZ]+ADD[t][rid]);
    }
    return (ll)ret;
}
    ...
    while(q--){
        ...
        if(t<=2){
            ...
            if(t==3){
                ...
                rep(i,2)rep(j,2){
                    ll va,vb;
                    if(i) va = max_query(0,l[0],r[0]);
                    else va = min_query(0,l[0],r[0]);

                    if(j) vb = max_query(1,l[1],r[1]);
                    else vb = min_query(1,l[1],r[1]);
