...
struct tree {
  ...
  int head[300030], num, siz[300030], zson[300030], tp[300030], f[300030],
      dep[300030], id[300030], idnum, dy[300030], rt[300030];
  inline void add(int from, int to) {
    bi[++num] = bian{head[from], to};
    head[from] = num;
  }
...
void merge(int x, int y) {
  int s = T.dy[lower_bound(a + 1, a + 1 + acnt, T.id[x]) - a],
      t = T.dy[upper_bound(a + 1, a + 1 + acnt, T.id[y]) - a - 1];
  if (T.id[x] <= T.id[s] && T.id[s] <= T.id[y] && T.id[x] <= T.id[t] &&
      T.id[t] <= T.id[y])
    while (find(t) != find(s)) f[find(t)] = find(fa[find(t)]);
}
int main() {
  ...
  for (int i = 1; i <= G.cnt; ++i)
    if (!T.dep[i]) T.rt[0] = i, T.dfs1(i, 0);
  for (int i = 1; i <= G.cnt; ++i)
    if (T.dep[i] == 1) T.dfs2(i, i);
  for (int t = 1; t <= q; ++t) {
    n1 = read(), m1 = read();
    for (int i = 1; i <= n1; ++i) {
    ...
    for (int i = 1; i <= cnt; ++i) {
      if (!tp || T.rt[st[tp]] != T.rt[a[i]]) {
        while (tp > 1)
          g.add(st[tp - 1], st[tp]), g.add(st[tp], st[tp - 1]), --tp;
        st[tp = 1] = T.rt[a[i]];
        if (!vis[T.rt[a[i]]])
          vis[T.rt[a[i]]] = 1, a[++acnt] = T.rt[a[i]];
        else
          continue;
      }
    ...
    if (flag)
      ...
      printf("YES\n"), (R += t) >= n ? R -= n : 0;
    for (int i = 1; i <= acnt; ++i)
      vis[a[i]] = g.head[a[i]] = g.dfn[a[i]] = g.low[a[i]] = 0;
    g.num = 0;
    g.cnt = 0;
    g.dfnum = 0;
    cnt = 0;
