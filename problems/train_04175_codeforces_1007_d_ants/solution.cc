...
struct twosat {
  ...
  bool ac() {
    for (int i = 0; i < 2 * n; ++i)
      if (!vis[i]) dfs1(i);
    for (int i = 0; i < 2 * n; ++i)
      for (int j : a1[i]) a2[j].push_back(i);
    for (int i = 2 * n - 1; i >= 0; --i)
      if (vis[st[i]]) dfs2(st[i], i);
...
int main() {
  ...
  for (int i = 0; i < 2 * m; ++i) {
    ...
    auto upd = [&](int l, int r) {
      for (l += n, r += n + 1; l < r; ++l /= 2, r /= 2) {
        if (l & 1) st[l].push_back(i);
        if (r & 1) st[r - 1].push_back(i);
      }
    };
    for (; pr[ui] != pr[vi]; ui = p[pr[ui]]) {
      if (d[pr[ui]] < d[pr[vi]]) swap(ui, vi);
      upd(ap[pr[ui]], ap[ui]);
    }
    if (d[ui] > d[vi]) swap(ui, vi);
    upd(ap[ui] + 1, ap[vi]);
  }
  ts.n = m;
  for (int i = 1; i < 2 * n; ++i) {
    for (int j : st[i]) {
      if (pv[i]) {
        ts.imp(j, (2 * (pv[i]) + 1));
        ts.imp((2 * (pv[i])), (2 * (ts.n)));
      }
      ts.imp(j, (2 * (ts.n)));
      pv[i] = ts.n++;
    }
    if (i < n) pv[2 * i] = pv[2 * i + 1] = pv[i];
  }
  if (!ts.ac()) {
    cout << "NO";
    return 0;
  ...
  cout << "YES\n";
  for (int i = 0; i < m; ++i) cout << 2 - ts.ans[i] << "\n";
}
