...
inline void link(int u, int v, int l, int r) {
  addedge(u + 2, v + 2, r - l), deg[u] -= l, deg[v] += l;
}
inline void reduce(int x) {
  for (int &i = first[x]; i && e[i].v <= 2; i = next[i])
    ;
}
int main() {
  int i;
...
int main() {
  ...
  A -= B, B *= n;
  for (i = 0; i < n; ++i)
    scanf("%d%d", &x0, &y0), X.D[i] = std::pair<int, int>(x0, i),
                             Y.D[i] = std::pair<int, int>(y0, i);
  cX = X.Discretize(n), cY = Y.Discretize(n);
  for (i = 0; i < n; ++i)
    ++cx[x[i] = X.F[i]], ++cy[y[i] = Y.F[i]],
        F::link(x[i] + 3, y[i] + 3 + cX, 0, 1);
  memcpy(lx, cx, cX << 2), memcpy(ly, cy, cY << 2);
  for (; q; --q)
    if (scanf("%d%d%d", &op, &r, &v), op == 1) {
      x0 = std::lower_bound(X.D, X.D + cX, std::pair<int, int>(r, 0)) - X.D;
      if (X.D[x0].first == r) down(lx[x0], (cx[x0] + v) / 2);
    } else {
      y0 = std::lower_bound(Y.D, Y.D + cY, std::pair<int, int>(r, 0)) - Y.D;
